...
class KafkaSSE {
     ...
     *                   kafkajs consumer constructor.  group.id and
     ...
     *                   kafkajs Kafka message and returns the same kafkaMessage, with
     ...
     *                   should match an event fired by the kafkajs
    ...
    constructor(req, res, options) {
        ...
        // If we won't be responding to any Kafka events,
        // there is no need to set up an event emitter.
    ...
    _init(assignments, atTimestamp) {
        ...
        .then(() => {
            this.log.info({ config: this.kafkaConfig }, 'Connecting to Kafka...');
            return utils.createKafkaConsumerAsync(this.kafkaConfig);
        })

        // Save the consumer and register any configured kafka event handlers
        .then(consumer => {
            // Save our consumer.
        ...
        .then(async () => {
            this.log.info('Fetching topic metadata...');
            const admin = this.kafkaConsumer.kafka.admin();
            await admin.connect();
            const metadata = await admin.fetchTopicMetadata();
            await admin.disconnect();
            this.log.info({ topics: metadata.topics.map(t => t.name) }, 'Available topics fetched');
            this.availableTopics = utils.getAvailableTopics(
                metadata.topics,
                this.options.allowedTopics
            );
            this.log.info({ availableTopics: this.availableTopics }, 'Filtered available topics');
        ...
        .then(async () => {
            this.log.info(
                ...
                `Subscribing to topic partitions: ${this.topics.join(', ')}`
            );

            await this.kafkaConsumer.assign({ partitions: this.assignments });
            this.log.info('Kafka consumer assigned to partitions successfully');
    ...
    _loop(startTime) {
        ...
        .then((kafkaMessage) => {
            ...
            this.log.debug(
                { offset: kafkaMessage.offset, topic: kafkaMessage.topic, partition: kafkaMessage.partition },
                'Sending message to SSE client'
            );
    ...
    /**
     * Given a kafkaMessage consumed from kafkajs KafkaConsumer,
    ...
    _updateLatestOffsetsMap(kafkaMessage) {
        ...
        if (this.options.useTimestampForId) {
            this.latestOffsetsMap[`${kafkaMessage.topic}/${kafkaMessage.partition}`].timestamp = Number(kafkaMessage.timestamp) + 1;
        }
        else {
            this.latestOffsetsMap[`${kafkaMessage.topic}/${kafkaMessage.partition}`].offset = Number(kafkaMessage.offset) + 1;
        }
        ...
        return new P((resolve, reject) => {
            ...
            const consumeCallback = async () => {
                try {
                    ...
                    const messages = consumer.consume(1);
                    ...
                    const deserialized = await this._deserialize(kafkaMessage);
                    const filtered = await this._filter(deserialized);
                    resolve(filtered);
                } catch (e) {
                    // Handle KafkaJS errors
                    if (e.type === 'NETWORK_EXCEPTION' || e.type === 'TRANSPORT') {
                        this.log.warn({ err: e }, 'Kafka network error, retrying...');
                        await P.delay(this.idleDelayMs);
                        resolve();
                    } else if (e.name === 'KafkaJSError' && e.code === 'NO_ERROR') {
                        // End of partition
                        this.log.trace('Reached end of partition');
                        await P.delay(this.idleDelayMs);
                        resolve();
                    } else {
                        this.log.error({ err: e }, 'Error consuming from Kafka');
                        reject(e);
                    }
                }
            };
    ...
    disconnect(reason) {
        ...
        const disconnectKafkaPromise = P.resolve().then(async () => {
            ...
            try {
                ...
                this.log.warn({ err: e }, "KafkaSSE disconnect: Error disconnecting Kafka consumer");
            }
        });
