...
class KafkaSSE {
    constructor(req, res, options) {
        ...
        this.log = {
            debug: (...args) => log.debug(`[${this.id}]`, ...args),
            info: (...args) => log.info(`[${this.id}]`, ...args),
            warn: (...args) => log.warn(`[${this.id}]`, ...args),
            error: (...args) => log.error(`[${this.id}]`, ...args),
            trace: (...args) => log.trace(`[${this.id}]`, ...args),
        };
    ...
    _buildKafkaConfig() {
        return Object.assign(
            { 'metadata.broker.list': 'localhost:9092', 'client.id': `entitybase-sse-${this.id}` },
            this.options.kafkaConfig,
            { 'enable.auto.commit': false, 'group.id': `entitybase-sse-${this.id}` }
        );
    }

    connect(assignments, atTimestamp) {
        this.log.debug('connect() called, starting _init');
        this._init(assignments, atTimestamp)
            .then(() => {
                this.log.debug('_init complete, starting _start');
                return this._start();
            })
            .catch((e) => {
                this.log.debug({ err: e.message }, 'catch block got error');
            ...
            .catch((e) => {
                this.log.debug({ err: e.message }, 'second catch block got error');
                this._handleConnectError(e);
            })
            .finally(() => {
                this.log.debug('finally block executing');
                if (!this.isFinished) {
    ...
    async _init(assignments, atTimestamp) {
        ...
        if (this.availableTopics.length === 0 && !this.options.allowedTopics) {
    ...
    async _start() {
        this.log.debug('_start() called - about to set headers');
        const responseHeaders = this.options.headers || {};
        responseHeaders['access-control-allow-origin'] = '*';
        responseHeaders['access-control-allow-methods'] = 'GET, OPTIONS';
        responseHeaders['access-control-allow-headers'] = 'Content-Type';
        this.log.debug({ responseHeaders }, 'SSE response headers');
        ...
        const onMessage = async (message, offsets) => {
            if (this.isFinished || this._resFinished()) return;
            this.log.debug({ offset: message.message?._kafka?.offset }, 'Sending message to SSE');
            await this.sse.send('message', message, offsets).catch((e) => {
                this.log.warn({ err: e }, 'Error sending SSE message');
            });
        };
        
        this.messageHandler.start(onMessage);
        this.log.info('SSE started, starting consumer');
    ...
    _handleConnectError(e) {
        ...
        this.log.error({ err: e, message: e.message, stack: e.stack, statusCode: e.statusCode || 500 }, 'KafkaSSE connect error');
        ...
        try {
            ...
            this.log.error({ err }, 'Failed to write error response');
        }
    }
